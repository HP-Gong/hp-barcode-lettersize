   ...
   public function __construct(){
	  ...
	  add_action('init', array($this, 'check_versions'));
	   ...
	   public static function create_barcode_scripts(){
		...
		wp_register_script('print-barcode', hp_barcode_lettersize_url_p .'js/print-barcode.js', array('jquery'));
		...
		wp_enqueue_script('print-barcode');
	  ...
	  if($_POST['remove_bar']) {
	  global $wpdb;
	  $wpdb->query("DELETE FROM {$wpdb->prefix}woo_bar1");
	  }
	  }
	  ...
	  <img src="'.sanitize_text_field($row2->img_base64).'" name="remove_bar" alt="'.sanitize_text_field($row2->id).'" style="width: 160px; height: 70px; background-color: white; border: 0px solid #021a40; padding: 4px 4px 4px 4px;"></td>';
	  ...
	  echo '<br>';
	  $result3 = $wpdb->get_results("SELECT * FROM {$wpdb->prefix}woo_bar1");
	  foreach($result3 as $row3){
	  echo '<input type="hidden" name="remove_bar" value="'.sanitize_text_field($row3->id).'" />';
	  }
	  ...
	  if($_POST['remove_bar']) {
	  global $wpdb;
	  $wpdb->query("DELETE FROM {$wpdb->prefix}woo_bar2");
	  }
	  }
	  ...
	  echo '<br>';
	  $result3 = $wpdb->get_results("SELECT * FROM {$wpdb->prefix}woo_bar2");
	  foreach($result3 as $row3){
	  echo '<input type="hidden" name="remove_bar" value="'.sanitize_text_field($row3->id).'" />';
	  }
	  ...
	  if($_POST['remove_bar']) {
	  global $wpdb;
	  $wpdb->query("DELETE FROM {$wpdb->prefix}woo_bar3");
	  }
	  }
	  ...
	  echo '<br>';
	  $result3 = $wpdb->get_results("SELECT * FROM {$wpdb->prefix}woo_bar3");
	  foreach($result3 as $row3){
	  echo '<input type="hidden" name="remove_bar" value="'.sanitize_text_field($row3->id).'" />';
	  }
