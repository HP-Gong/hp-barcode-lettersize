...
function hp_barcode_img_s($sku){
	...
	if ( strtolower($code_type) == "code128a" ) { 	// Code128a
		...
		$txtW1 = imagefontwidth( $font ) * strlen($value);
		...
		imagestring($bar_img, $font, $xL1, $y1, $value, $black);  
		...
		$y1 = 55; // y Location of imagestring
		$txtW1 = imagefontwidth( $font ) * strlen($upper_text);
		$xL1 = ( $xMax1 - $xMin1 - $txtW1 ) / 2 + $xMin1 + $font;
		
		imagefill( $bar_img, 0, 0, $white );
		imagecopyresized($bar_img, $bars, 0, 15, 0, 0, $cur_size, $img_height, $cur_size, $img_height);	 
		imagestring($bar_img, $font, $xL1, $y1, $upper_text, $black); 
		...
		$xMax1 = 78;
		$xMin1 = 78;  // x Location of imagestring 72
		$y1 = 55; // y Location of imagestring
		$txtW1 = imagefontwidth( $font ) * strlen($sku);
		$xL1 = ( $xMax1 - $xMin1 - $txtW1 ) / 2 + $xMin1 + $font;
		
		imagefill( $bar_img, 0, 0, $white );
		imagecopyresized($bar_img, $bars, 45, 15, 0, 0, $cur_size, $img_height, $cur_size, $img_height);	 
		imagestring($bar_img, $font, $xL1, $y1, $sku, $black); 
