...
function hp_barcode_img_ts($string, $sku) {
	...
	if ( strtolower($code_type) == "code128a" ) { 	// Code128a
		...
		$value = strtoupper($sku); // Code 128A doesn't support lower case
		...
		$bar_img = imagecreate(($cur_size+2) < 160 ? 160 : ($cur_size+2), 70);
		...
		$txtW1 = imagefontwidth( $font ) * strlen($string);
		$xL1 = ( $xMax1 - $xMin1 - $txtW1 ) / 2 + $xMin1 + $font;
		
		$xMax2 = 80;
		$xMin2 = 80;// x Location of imagestring 80	
		$y2 = 55; // y Location of imagestring
		$txtW2 = imagefontwidth( $font ) * strlen($value);
		$xL2 = ( $xMax2 - $xMin2 - $txtW2 ) / 2 + $xMin2 + $font;
		
		imagefill( $bar_img, 0, 0, $white );
		imagecopyresized($bar_img, $bars, 12, 18, 0, 0, $cur_size, $img_height, $cur_size, $img_height);	 	 
		imagestring($bar_img, $font, $xL1, $y1, $string, $black); 
		imagestring($bar_img, $font, $xL2, $y2, $value, $black); 
		...
		$bar_img = imagecreate(($cur_size+2) < 10 ? 10 : ($cur_size+2), 70);
		$black = imagecolorallocate ($bar_img, 0, 0, 0);
		$white = imagecolorallocate ($bar_img, 255, 255, 255);
		...
		$xMax1 = 90;
		$xMin1 = 90;  // x Location of imagestring 90
		$y1 = 2; // y Location of imagestring
		$txtW1 = imagefontwidth( $font ) * strlen($string);
		$xL1 = ( $xMax1 - $xMin1 - $txtW1 ) / 2 + $xMin1 + $font;
		
		$xMax2 = 90;
		$xMin2 = 90;// x Location of imagestring 90	
		$y2 = 55; // y Location of imagestring
		$txtW2 = imagefontwidth( $font ) * strlen($upper_text);
		$xL2 = ( $xMax2 - $xMin2 - $txtW2 ) / 2 + $xMin2 + $font;
		
		imagefill( $bar_img, 0, 0, $white );
		imagecopyresized($bar_img, $bars, 0, 15, 0, 0, $cur_size, $img_height, $cur_size, $img_height);	 
		imagestring($bar_img, $font, $xL1, $y1, $string, $black); 
		imagestring($bar_img, $font, $xL2, $y2, $upper_text, $black); 
		...
		for ( $x = 1; $x <= strlen($sku); $x++ ) {
			for ( $y = 0; $y < count($b_array1); $y++ ) {
				if ( substr($sku, ($x-1), 1) == $b_array1[$y] )
					$temp[$x] = $b_array2[$y];
			}
		}

		for ( $x=1; $x<=strlen($sku); $x+=2 ) {
		...
		$xMax1 = 78;
		$xMin1 = 78;  // x Location of imagestring 78
		$y1 = 2; // y Location of imagestring
		$txtW1 = imagefontwidth( $font ) * strlen($string);
		$xL1 = ( $xMax1 - $xMin1 - $txtW1 ) / 2 + $xMin1 + $font;
		
		$xMax2 = 78;
		$xMin2 = 78;// x Location of imagestring 78	
		$y2 = 55; // y Location of imagestring
		$txtW2 = imagefontwidth( $font ) * strlen($sku);
		$xL2 = ( $xMax2 - $xMin2 - $txtW2 ) / 2 + $xMin2 + $font;
		
		imagefill( $bar_img, 0, 0, $white );
		imagecopyresized($bar_img, $bars, 45, 15, 0, 0, $cur_size, $img_height, $cur_size, $img_height);	 
		imagestring($bar_img, $font, $xL1, $y1, $string, $black); 
		...
		$bar_img = imagecreate(($cur_size+2) < 145 ? 145 : ($cur_size+2), 70);
		$black = imagecolorallocate ($bar_img, 0, 0, 0);
		$white = imagecolorallocate ($bar_img, 255, 255, 255);
		...
		$xMax1 = 72;
		$xMin1 = 72;  // x Location of imagestring 72
		$y1 = 2; // y Location of imagestring
		$txtW1 = imagefontwidth( $font ) * strlen($string);
		$xL1 = ( $xMax1 - $xMin1 - $txtW1 ) / 2 + $xMin1 + $font;
		
		$xMax2 = 72;
		$xMin2 = 72;// x Location of imagestring 72	
		$y2 = 55; // y Location of imagestring
		$txtW2 = imagefontwidth( $font ) * strlen($upper_text);
		$xL2 = ( $xMax2 - $xMin2 - $txtW2 ) / 2 + $xMin2 + $font;
		
		imagefill( $bar_img, 0, 0, $white );
		imagecopyresized($bar_img, $bars, 10, 15, 0, 0, $cur_size, $img_height, $cur_size, $img_height);	 
		imagestring($bar_img, $font, $xL1, $y1, $string, $black); 
		imagestring($bar_img, $font, $xL2, $y2, $upper_text, $black); 
